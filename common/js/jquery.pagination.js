...
jQuery.fn.pagination = function(maxentries, opts){
	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			var pagetext = '<input id="pagevalue" size="4" value="'+(current_page+1)+'" type="number"   onpropertychange="OnPropChanged (event)">';
			var totallist="<span class='page-sum'>共<strong class='allPage'>"+Math.ceil(maxentries/opts.items_per_page)+"</strong>页</span>"
            var toPage='<span><a id="search" href="#">跳转</a></span>';
             $(totallist).appendTo(panel);
            $(pagetext).appendTo(panel);
            
            $(toPage).appendTo(panel);
            
            $('#pagevalue').bind('input propertychange', function() {
            	var r= /^[+-]?[1-9]?[0-9]*\.[0-9]*$/;
			    if($("#pagevalue").val()==0)
                   $("#pagevalue").val("")
                if(r.test($("#pagevalue").val())){$("#pagevalue").val("")}
			});
